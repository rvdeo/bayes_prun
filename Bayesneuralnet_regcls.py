...
class MCMC:
    ...
    def rmse(self, predictions, targets):
        ...
        )  # for one step prediction or one output neuron only
    ...
    def prune_weights(self, w, starting_w):
    ...
    def re_sampler(self, w_limit, tau_limit, starting_w):
        ...
        w = starting_w #np.random.randn(w_size)
        w_proposal = np.random.randn(w_size)

        w_proposal = self.prune_weights( w_proposal, starting_w)
        pos_w[0] = w_proposal
        ...
        for i in range(samples - 1):
            ...
            if (self.use_langevin_gradients is True) and (lx < self.l_prob):
                ...
                w_proposal = self.prune_weights( w_proposal, starting_w) # post prunning model
                
                w_prop_gd = neuralnet.langevin_gradient(
                    self.traindata, w_proposal.copy(), self.sgd_depth
                )
                w_prop_gd = self.prune_weights( w_prop_gd, starting_w)# post prunning model
